...
let refreshDOMTable = () => {
    ...
    for (let i = 0; i < cmsTableKeys.length; i++) {
        ...
        let currentAdderssCol = document.createElement('div');
        ...
        currentAdderssCol.className = 'cms-table-column cms-address';
        ...
        currentAdderssCol.innerHTML = cmsTable[cmsTableKeys[i]].address;
        ...
        currentRow.appendChild(currentAdderssCol);
...
let deleteUserFromTable = (userName) => {
    let tempTable = {};
    let cmsTablelKeys = Object.keys(cmsTable);
    for (let i = 0; i < cmsTableKeys.length; i++) {
        if (userName !== cmsTableKeys[i]) {
            tempTable[cmsTableKeys[i]] = cmsTablel[cmsTableKeys[i]];
        }
    }
    cmsTable = tempTable;
    localStorage.setItem(tableKey, JSON.stringify(cmsTable));
